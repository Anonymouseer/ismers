...
import PositionRequisitions from '../components/PositionRequisitions';
...
export default function JobOrderManagementPage() {
  ...
  const viewMode = searchParams.get('view') || 'board'; // 'board' | 'requisitions'
      ...
      <div className={`main${collapsed ? ' collapsed' : ''}`}>
        ...
        <div className="title-row">
          <div>
            ...
            <h1 className="page-title">
              {viewMode === 'requisitions' ? 'Position Requisitions (PRF)' : 'Job Order Dispatch Board'}
            </h1>
            <div className="page-sub">
              {viewMode === 'requisitions'
                ? `${store.stats.total} position requisition forms across ${store.stats.clientCount} clients`
        ...
        {viewMode === 'requisitions' ? (
          <PositionRequisitions
            jobOrders={store.jobOrders ?? []}
            onViewJobOrder={(ref) => store.openDetail(ref)}
          />
        ) : (
          <>
